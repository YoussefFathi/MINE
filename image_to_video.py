...
class Args:
    def __init__(self, checkpoint_path,data_path,output_dir,gpus,extra_config="{}"):
...
def main():
    ...
    all_dirs = ['2011_09_26_drive_0104_sync',
      '2011_09_26_drive_0106_sync',
      '2011_09_26_drive_0113_sync',
      '2011_09_26_drive_0117_sync',]
    # args = parser.parse_args()
    args = Args(checkpoint_path='/home/yafathi/projects/def-karray/yafathi/MINE/KITTI_Pretrained/checkpoint.pth',gpus=0,data_path='/home/yafathi/scratch/kitti_raw_data/2011_09_26', output_dir='/home/yafathi/projects/def-karray/yafathi/MINE/Videos/Kitti_Raw')
    ...
    for dir in all_dirs:
        current_dir = os.path.join(args.data_path,dir,"image_02","data")
        filesCount = 0
        # print(current_dir)
        # print(glob.glob(current_dir+"/*.png"))
        for file in glob.glob(current_dir+"/*.png"):
            if filesCount==20:
                filesCount=0
            if filesCount == 0:
                # print(args.path)
                # file = '/home/yafathi/scratch/kitti_raw_data/2011_09_26/2011_09_26_drive_0117_sync/image_02/data/0000000640.png'
                img_np = cv2.imread(file, cv2.IMREAD_COLOR)
                img_np = cv2.cvtColor(img_np, cv2.COLOR_BGR2RGB)
                args.output_dir = original_output + "/" + dir
                if(not os.path.exists(args.output_dir)):
                        os.makedirs(args.output_dir)
                video_generator = VideoGenerator(synthesis_task, config, config["logger"], img_np, args.output_dir)
                with torch.no_grad():
                    # path = '/home/yafathi/projects/def-karray/yafathi/MINE/Videos/Kitti_Raw/2011_09_26'
                    # if(not os.path.exists(path)):
                    #     os.makedirs(path)
                    video_generator.render_video(args.output_dir +"/"+os.path.basename(file)[0:-4])
                    # break
